...
#define bNextAnim    g_int_3
#define CurFrame2    g_int_4
#define NextFrame2   g_int_5
//#define NextFrame   
#define Ratio        g_float_0
#define Ratio2       g_float_1
#define MixRatio     g_float_2
// ===========================
[numthreads(256, 1, 1)]
void CS_Animation3D(int3 _iThreadIdx : SV_DispatchThreadID)
{
	...
	//2
	uint iFrameDataIndex2 = BoneCount * CurFrame2 + _iThreadIdx.x;
	uint iNextFrameDataIdx2 = BoneCount * NextFrame2 + _iThreadIdx.x;
	
	float4 vScale2 = lerp(g_arrFrameTrans[iFrameDataIndex2].vScale, g_arrFrameTrans[iNextFrameDataIdx2].vScale, Ratio2);
	float4 vTrans2 = lerp(g_arrFrameTrans[iFrameDataIndex2].vTranslate, g_arrFrameTrans[iNextFrameDataIdx2].vTranslate, Ratio2);
	float4 qRot2 = QuternionLerp(g_arrFrameTrans[iFrameDataIndex2].qRot, g_arrFrameTrans[iNextFrameDataIdx2].qRot, Ratio2);
	
	//mix
	if (bNextAnim)
	{
		vScale = lerp(vScale, vScale2,  MixRatio);
		vTrans = lerp(vTrans, vTrans2, MixRatio);
		qRot = QuternionLerp(qRot, qRot2, MixRatio);
	}
